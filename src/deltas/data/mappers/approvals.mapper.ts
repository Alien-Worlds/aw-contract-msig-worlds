...
// Mongo Mappers
export class ApprovalsMongoMapper extends MongoMapper<
  Approvals,
  ApprovalsMongoModel
> {
  constructor() {
    ...
    this.mappingFromEntity.set('requestedApprovals', {
      ...
      mapper: (values: Approval[]) =>
        values.map(value => new ApprovalMongoMapper().fromEntity(value)),
    });
  ...
  public toEntity(mongoModel: ApprovalsMongoModel): Approvals {
    ...
    return Approvals.create(
      proposal_name || '',
      requested_approvals?.map(value =>
        new ApprovalMongoMapper().toEntity(value)
      ) || [],
      provided_approvals?.map(value =>
        new ApprovalMongoMapper().toEntity(value)
      ) || [],
...
export class ApprovalMongoMapper extends MongoMapper<
  Approval,
  ApprovalMongoModel
> {
  constructor() {
  ...
  public toEntity(mongoModel: ApprovalMongoModel): Approval {
    const { level, time, _id, ...rest } = mongoModel;
...
// Raw mappers
export class ApprovalsRawMapper extends MapperImpl<
  Approvals,
  ApprovalsRawModel
> {
  ...
  public toEntity(rawModel: ApprovalsRawModel): Approvals {
    const { proposal_name, requested_approvals, provided_approvals, ...rest } =
      rawModel;

    return Approvals.create(
      proposal_name || '',
      requested_approvals?.map(value =>
        new ApprovalRawMapper().toEntity(value)
      ) || [],
      provided_approvals?.map(value =>
        new ApprovalRawMapper().toEntity(value)
      ) || [],
...
export class ApprovalRawMapper extends MapperImpl<Approval, ApprovalRawModel> {
  public fromEntity(entity: Approval): ApprovalRawModel {
    throw new Error('Method not implemented');
  }

  public toEntity(rawModel: ApprovalRawModel): Approval {
    const { level, time, ...rest } = rawModel;
